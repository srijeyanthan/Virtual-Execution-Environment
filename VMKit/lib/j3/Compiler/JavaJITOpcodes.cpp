...
static inline uint32 WREAD_U1(Reader& reader, bool init, uint32 &i,
		bool& wide) {
	if (wide) {
		wide = init;
		i += 2;
...
static inline sint32 WREAD_S1(Reader& reader, bool init, uint32 &i,
		bool &wide) {
	if (wide) {
		wide = init;
		i += 2;
 ...
 */
static uint8 sum[] = { 2, 3, 2, 3, 3, 2, 2, 2, 2, 2 };
static uint8 sum2[] = { 3, 3, 3, 3, 3, 3, 3, 5, 5, 3, 2, 3, 1, 1, 3, 3, 1, 1 };
...
void JavaJIT::findUnreachableCode(Reader& reader, uint32 codeLen) {
	...
	while (queue.size()) {
		...
		if (bytecode < IFEQ) {
		...
		} else if (bytecode >= GETSTATIC && bytecode <= MONITOREXIT
				&& bytecode != ATHROW) {
			...
			case JSR:
				...
				if (next_index < codeLen
						&& !opcodeInfos[next_index].isReachable) {
					opcodeInfos[next_index].isReachable = true;
					queue.push(next_index);
				}
				step = reader.readS2();
			...
			case JSR_W:
				...
				break;
			case WIDE:
				if (i_opcode + 1 < codeLen
						&& !opcodeInfos[i_opcode + 1].isReachable) {
			...
			case TABLESWITCH:
				...
				if (defaultIndex < codeLen
						&& !opcodeInfos[defaultIndex].isReachable) {
	...
	for (uint32 i = 0; i < codeLength; ++i) {
		...
		if (opinfo->newBlock) {
			...
			if (opinfo->handler) {
				...
				Value* javaExceptionPtr = getJavaExceptionPtr(
						getJavaThreadPtr(getMutatorThreadPtr()));
				Value* obj = new LoadInst(javaExceptionPtr, "pendingException",
						currentBlock);
				new StoreInst(obj, objectStack[0], "", currentBlock);
				// And clear the exception.
				new StoreInst(intrinsics->JavaObjectNullConstant,
						javaExceptionPtr, currentBlock);
		...
		case SIPUSH:
			push(
					ConstantExpr::getSExt(
							ConstantInt::get(Type::getInt16Ty(*llvmContext),
									reader.readS2()),
							Type::getInt32Ty(*llvmContext)), false);
			i += 2;
			break;
		...
		case ILOAD:
			push(
					new LoadInst(intLocals[WREAD_U1(reader, false, i, wide)],
							"", false, currentBlock), false);
			break;

		case LLOAD:
			push(
					new LoadInst(longLocals[WREAD_U1(reader, false, i, wide)],
							"", false, currentBlock), false);
			push(intrinsics->constantZero, false);
			break;
		...
		case FLOAD:
			push(
					new LoadInst(floatLocals[WREAD_U1(reader, false, i, wide)],
							"", false, currentBlock), false);
			break;

		case DLOAD:
			push(
					new LoadInst(doubleLocals[WREAD_U1(reader, false, i, wide)],
							"", false, currentBlock), false);
			push(intrinsics->constantZero, false);
			break;
		...
		case ALOAD:
			push(
					new LoadInst(objectLocals[WREAD_U1(reader, false, i, wide)],
							"", false, currentBlock), false);
			break;
		...
		case LLOAD_0:
			push(new LoadInst(longLocals[0], "", false, currentBlock), false);
			push(intrinsics->constantZero, false);
			break;

		case LLOAD_1:
			push(new LoadInst(longLocals[1], "", false, currentBlock), false);
			push(intrinsics->constantZero, false);
			break;

		case LLOAD_2:
			push(new LoadInst(longLocals[2], "", false, currentBlock), false);
			push(intrinsics->constantZero, false);
			break;

		case LLOAD_3:
			push(new LoadInst(longLocals[3], "", false, currentBlock), false);
			push(intrinsics->constantZero, false);
			break;

		case FLOAD_0:
			push(new LoadInst(floatLocals[0], "", false, currentBlock), false);
			break;

		case FLOAD_1:
			push(new LoadInst(floatLocals[1], "", false, currentBlock), false);
			break;

		case FLOAD_2:
			push(new LoadInst(floatLocals[2], "", false, currentBlock), false);
			break;

		case FLOAD_3:
			push(new LoadInst(floatLocals[3], "", false, currentBlock), false);
			break;

		case DLOAD_0:
			push(new LoadInst(doubleLocals[0], "", false, currentBlock), false);
			push(intrinsics->constantZero, false);
			break;

		case DLOAD_1:
			push(new LoadInst(doubleLocals[1], "", false, currentBlock), false);
			push(intrinsics->constantZero, false);
			break;

		case DLOAD_2:
			push(new LoadInst(doubleLocals[2], "", false, currentBlock), false);
			push(intrinsics->constantZero, false);
			break;

		case DLOAD_3:
			push(new LoadInst(doubleLocals[3], "", false, currentBlock), false);
			push(intrinsics->constantZero, false);
			break;

		case ALOAD_0:
			push(new LoadInst(objectLocals[0], "", false, currentBlock), false);
			break;

		case ALOAD_1:
			push(new LoadInst(objectLocals[1], "", false, currentBlock), false);
			break;

		case ALOAD_2:
			push(new LoadInst(objectLocals[2], "", false, currentBlock), false);
			break;

		case ALOAD_3:
			push(new LoadInst(objectLocals[3], "", false, currentBlock), false);
			break;
		...
		case BALOAD: {
			...
			push(
					new SExtInst(val, Type::getInt32Ty(*llvmContext), "",
							currentBlock), false);
			break;
		}
		...
		case CALOAD: {
			...
			Value* val = new LoadInst(ptr, "", currentBlock);
			push(
					new ZExtInst(val, Type::getInt32Ty(*llvmContext), "",
							currentBlock), false);
			break;
		}
		...
		case ASTORE: {
			CommonClass* cl = topTypeInfo();
			Instruction* V = new StoreInst(pop(),
					objectLocals[WREAD_U1(reader, false, i, wide)], false,
					currentBlock);
		...
		case ISTORE_0: {
			...
			if (val->getType() != Type::getInt32Ty(*llvmContext)) // int8 and int16
				val = new ZExtInst(val, Type::getInt32Ty(*llvmContext), "",
						currentBlock);
		...
		case AASTORE: {
			if (TheCompiler->hasExceptionsEnabled()) {
				...
				if (TheCompiler->isStaticCompiling()) {
					BasicBlock* endBlock = createBasicBlock(
							"end array store check");
					BasicBlock* checkBlock = createBasicBlock(
							"array store check");
					BasicBlock* exceptionBlock = createBasicBlock(
							"array store exception");
					Value* cmp = new ICmpInst(*currentBlock, ICmpInst::ICMP_EQ,
							val, intrinsics->JavaObjectNullConstant, "");

					BranchInst::Create(endBlock, checkBlock, cmp, currentBlock);
					currentBlock = checkBlock;

					Value* valVT = CallInst::Create(intrinsics->GetVTFunction,
							val, "", currentBlock);

					Value* objVT = CallInst::Create(intrinsics->GetVTFunction,
							obj, "", currentBlock);
					objVT = CallInst::Create(
							intrinsics->GetBaseClassVTFromVTFunction, objVT, "",
							currentBlock);
					...
					res = CallInst::Create(
							...
							VTArgs, "", currentBlock);
					BranchInst::Create(endBlock, exceptionBlock, res,
							currentBlock);

					currentBlock = exceptionBlock;
					throwRuntimeException(
							intrinsics->ArrayStoreExceptionFunction, VTArgs, 2);
					currentBlock = endBlock;
				} else {
					Value* objects[2] = { val, obj };
					res = CallInst::Create(intrinsics->CheckIfAssignable,
							objects, "", currentBlock);
				}
			...
			if (vmkit::Collector::needsWriteBarrier()) {
				ptr = new BitCastInst(ptr, intrinsics->ptrPtrType, "",
						currentBlock);
				val = new BitCastInst(val, intrinsics->ptrType, "",
						currentBlock);
				obj = new BitCastInst(obj, intrinsics->ptrType, "",
						currentBlock);
				Value* args[3] = { obj, ptr, val };
				CallInst::Create(intrinsics->ArrayWriteBarrierFunction, args,
						"", currentBlock);
			} else {
				new StoreInst(val, ptr, false, currentBlock);
			}
		...
		case BASTORE: {
			...
			if (val->getType() != Type::getInt8Ty(*llvmContext)) {
				val = new TruncInst(val, Type::getInt8Ty(*llvmContext), "",
						currentBlock);
			}
			Value* index = pop();
			Value* obj = pop();
		...
		case CASTORE: {
			...
			if (type == Type::getInt32Ty(*llvmContext)) {
				val = new TruncInst(val, Type::getInt16Ty(*llvmContext), "",
						currentBlock);
			} else if (type == Type::getInt8Ty(*llvmContext)) {
				val = new ZExtInst(val, Type::getInt16Ty(*llvmContext), "",
						currentBlock);
			}
			Value* index = pop();
			Value* obj = pop();
		...
		case SASTORE: {
			...
			if (type == Type::getInt32Ty(*llvmContext)) {
				...
				val = new SExtInst(val, Type::getInt16Ty(*llvmContext), "",
						currentBlock);
			}
			Value* index = pop();
			Value* obj = pop();
		...
		case IDIV: {
			...
			currentBlock = ifTrue;
			node->addIncoming(
					BinaryOperator::CreateSub(intrinsics->constantZero, val1,
		...
		case LDIV: {
			...
			if (TheCompiler->hasExceptionsEnabled()) {
				Value* cmp = new ICmpInst(*currentBlock, ICmpInst::ICMP_EQ,
						val2, intrinsics->constantLongZero, "");
				BasicBlock* ifFalse = createBasicBlock("non null div");
				BasicBlock* ifTrue = createBasicBlock("null div");
		...
		case INEG:
			push(
					BinaryOperator::CreateSub(intrinsics->constantZero,
							popAsInt(), "", currentBlock), false);
			break;

		case LNEG: {
			pop();
			push(
					BinaryOperator::CreateSub(intrinsics->constantLongZero,
		...
		case FNEG:
			push(
					BinaryOperator::CreateFSub(
							intrinsics->constantFloatMinusZero, pop(), "",
							currentBlock), false);
			break;
		...
		case DNEG: {
			...
			push(
					BinaryOperator::CreateFSub(
							intrinsics->constantDoubleMinusZero, pop(), "",
							currentBlock), false);
			push(intrinsics->constantZero, false);
			break;
		}
		...
		case ISHL: {
			Value* val2 = popAsInt();
			Value* val1 = popAsInt();
			Value* mask = ConstantInt::get(Type::getInt32Ty(*llvmContext),
					0x1F);
		...
		case LSHL: {
			Value* val2 = new ZExtInst(pop(), Type::getInt64Ty(*llvmContext),
					"", currentBlock);
		...
		case IOR: {
			Value* val2 = popAsInt();
			Value* val1 = popAsInt();
			push(BinaryOperator::CreateOr(val1, val2, "", currentBlock), false);
			break;
		}
		...
		case I2L:
			push(
					new SExtInst(pop(), llvm::Type::getInt64Ty(*llvmContext),
							"", currentBlock), false);
			push(intrinsics->constantZero, false);
			break;
		...
		case I2F:
			push(
					new SIToFPInst(pop(), llvm::Type::getFloatTy(*llvmContext),
							"", currentBlock), false);
			break;

		case I2D:
			push(
					new SIToFPInst(pop(), llvm::Type::getDoubleTy(*llvmContext),
							"", currentBlock), false);
			push(intrinsics->constantZero, false);
			break;
		...
		case L2I:
			pop();
			push(
					new TruncInst(pop(), llvm::Type::getInt32Ty(*llvmContext),
							"", currentBlock), false);
			break;
		...
		case F2I: {
			...
			PHINode* node = PHINode::Create(
					llvm::Type::getInt32Ty(*llvmContext), 4, "", res);
			node->addIncoming(intrinsics->constantZero, currentBlock);
			...
			currentBlock = cont;
			llvm::Value* newVal = new FPToSIInst(val,
					Type::getInt32Ty(*llvmContext), "", currentBlock);
		...
		case F2L: {
			...
			PHINode* node = PHINode::Create(Type::getInt64Ty(*llvmContext), 4,
					"", res);
		...
		case F2D:
			push(
					new FPExtInst(pop(), llvm::Type::getDoubleTy(*llvmContext),
							"", currentBlock), false);
			push(intrinsics->constantZero, false);
			break;
		...
		case D2I: {
			...
			PHINode* node = PHINode::Create(Type::getInt32Ty(*llvmContext), 4,
					"", res);
			node->addIncoming(intrinsics->constantZero, currentBlock);
		...
		case D2F:
			pop(); // remove the 0 on the stack
			push(
					new FPTruncInst(pop(), llvm::Type::getFloatTy(*llvmContext),
							"", currentBlock), false);
			break;
		...
		case I2B: {
			...
			if (val->getType() == Type::getInt32Ty(*llvmContext)) {
				val = new TruncInst(val, llvm::Type::getInt8Ty(*llvmContext),
						"", currentBlock);
			}
			push(
					new SExtInst(val, llvm::Type::getInt32Ty(*llvmContext), "",
							currentBlock), false);
			break;
		}
		...
		case I2C: {
			Value* val = pop();
			if (val->getType() == Type::getInt32Ty(*llvmContext)) {
				val = new TruncInst(val, llvm::Type::getInt16Ty(*llvmContext),
						"", currentBlock);
			}
			push(
					new ZExtInst(val, llvm::Type::getInt32Ty(*llvmContext), "",
							currentBlock), false);
			break;
		}
		...
		case JSR: {
			...
			Value* expr = ConstantExpr::getIntToPtr(
					ConstantInt::get(Type::getInt64Ty(*llvmContext),
							uint64_t(index)), intrinsics->JavaObjectType);
			push(expr, false);
			branch(opcodeInfos[tmp + reader.readS2()], currentBlock);
			i += 2;
			break;
		...
		case RET: {
			...
			i += 1;
			Value* _val = new LoadInst(objectLocals[local], "", false,
					currentBlock);
			Value* val = new PtrToIntInst(_val, Type::getInt32Ty(*llvmContext),
					"", currentBlock);
			SwitchInst* inst = SwitchInst::Create(val, jsrs[0], jsrs.size(),
					currentBlock);

			uint32 index = 0;
			for (std::vector<BasicBlock*>::iterator i = jsrs.begin(), e =
					jsrs.end(); i != e; ++i, index += 2) {
				inst->addCase(
						ConstantInt::get(Type::getInt32Ty(*llvmContext),
								index | 1), *i);
			}
		...
		case LOOKUPSWITCH: {
			...
			for (uint32 cur = 0; cur < nbs; ++cur) {
				Value* val = ConstantInt::get(Type::getInt32Ty(*llvmContext),
						reader.readU4());
				i += 4;
				Value* cmp = new ICmpInst(*currentBlock, ICmpInst::ICMP_EQ, val,
						key, "");
				BasicBlock* falseBlock = createBasicBlock(
						"continue lookupswitch");
				Opinfo& info = opcodeInfos[tmp + reader.readU4()];
				i += 4;
				branch(cmp, info.newBlock, falseBlock, currentBlock, info);
				currentBlock = falseBlock;
		...
		case ANEWARRAY: {
			...
			if (bytecode == NEWARRAY) {
				...
				if (TheCompiler->isStaticCompiling()
					...
					TheVT = CallInst::Create(
							intrinsics->GetVTFromClassArrayFunction, valCl, "",
							currentBlock);
				} else {
				...
				if (cl && (!cl->isClass() || cl->asClass()->isResolved())) {
					...
					const UTF8* arrayName = JCL->constructArrayName(1,
							cl->name);
					...
					Value* args[3] = { TheCompiler->getNativeClass(
							compilingClass), ConstantInt::get(
							Type::getInt32Ty(*llvmContext), index),
							Constant::getNullValue(Ty) };
					TheVT = CallInst::Create(intrinsics->GetArrayClassFunction,
							args, "", currentBlock);
				}
			...
			if (TheCompiler->hasExceptionsEnabled()) {
				Value* cmp = new ICmpInst(*currentBlock, ICmpInst::ICMP_SLT,
						arg1, intrinsics->constantZero, "");
				...
				currentBlock = BB1;
				throwRuntimeException(
						intrinsics->NegativeArraySizeExceptionFunction, arg1);
				currentBlock = BB2;
				...
				BranchInst::Create(BB1, BB2, cmp, currentBlock);
				currentBlock = BB1;
				throwRuntimeException(intrinsics->OutOfMemoryErrorFunction,
						arg1);
				currentBlock = BB2;
			}
			...
			Value* mult = BinaryOperator::CreateShl(arg1, sizeElement, "",
					currentBlock);
			Value* size = BinaryOperator::CreateAdd(
					intrinsics->JavaArraySizeConstant, mult, "", currentBlock);
			TheVT = new BitCastInst(TheVT, intrinsics->ptrType, "",
					currentBlock);
			Instruction* res = invoke(intrinsics->VTAllocateFunction, size,
					TheVT, "", currentBlock);
			...
			Value* GEP = GetElementPtrInst::Create(cast, gep4, "",
					currentBlock);

			arg1 = new IntToPtrInst(arg1, intrinsics->ptrType, "",
					currentBlock);
			new StoreInst(arg1, GEP, currentBlock);

			addHighLevelType(res, dcl ? dcl : upcalls->ArrayOfObject);
			res = new BitCastInst(res, intrinsics->JavaObjectType, "",
					currentBlock);
		...
		case INSTANCEOF: {
			...
			PHINode* node = PHINode::Create(Type::getInt1Ty(*llvmContext), 2,
					"", endBlock);
			...
			if (checkcast) {
				...
				throwRuntimeException(intrinsics->ClassCastExceptionFunction,
						args, 2);
				currentBlock = ifFalse;
			} else {
				BasicBlock* ifFalse = createBasicBlock("false type compare");
				BranchInst::Create(endBlock, ifFalse, cmp, currentBlock);
				node->addIncoming(ConstantInt::getFalse(*llvmContext),
						currentBlock);
				currentBlock = ifFalse;
			}
			...
			if (!cl || TheCompiler->isStaticCompiling()) {
				TheVT = CallInst::Create(
						intrinsics->GetVTFromCommonClassFunction, clVar, "",
						currentBlock);
			} else {
			...
			if (cl) {
				if (cl->isSecondaryClass()) {
					if (TheCompiler->isStaticCompiling())
						res = CallInst::Create(
								intrinsics->IsSecondaryClassFunction, classArgs,
								"", currentBlock);
					else
						res = CallInst::Create(
								intrinsics->IsSecondaryClassFunctionInner,
								classArgs, "", currentBlock);
				} else {
					Value* inDisplay = CallInst::Create(
							intrinsics->GetDisplayFunction, objVT, "",
							currentBlock);

					uint32 depth = cl->virtualVT->depth;
					ConstantInt* CI = ConstantInt::get(
							Type::getInt32Ty(*llvmContext), depth);
					Value* displayArgs[2] = { inDisplay, CI };
					Value* VTInDisplay = CallInst::Create(
							intrinsics->GetVTInDisplayFunction, displayArgs, "",
							currentBlock);

					res = new ICmpInst(*currentBlock, ICmpInst::ICMP_EQ,
							VTInDisplay, TheVT, "");
				}
			} else {
				if (TheCompiler->isStaticCompiling())
					...
					res = CallInst::Create(
							intrinsics->IsSubclassOfFunctionInner, classArgs,
							"", currentBlock);
			}
			...
			if (checkcast) {
				...
				pop();
				push(
						new ZExtInst(node, Type::getInt32Ty(*llvmContext), "",
								currentBlock), false);
			}
		...
		case MULTIANEWARRAY: {
			...
			Value** args = (Value**) allocator.Allocate(
					sizeof(Value*) * (dim + 2));
			args[0] = valCl;
			...
			push(
					invoke(intrinsics->MultiCallNewFunction, Args, "",
							currentBlock), false,
					dcl ? dcl : upcalls->ArrayOfObject);
			break;
		}
		...
		default: {
			fprintf(stderr,
					"I haven't verified your class file and it's malformed:"
...
void JavaJIT::exploreOpcodes(Reader& reader, uint32 codeLength) {
	...
	for (uint32 i = 0; i < codeLength; ++i) {
		...
		//std::cout<<"This is the bye code  - "<<bytecode<<std::endl;
		...
		case ASTORE_0:
			if (!isStatic(compilingMethod->access))
				overridesThis = true;
			break;
		...
		case IFNONNULL: {
			...
			if (index < i)
				opcodeInfos[index].backEdge = true;
			i += 2;
...
static void updateStack(std::vector<uint8>& stack, Signdef* signature,
		uint8 bytecode) {
	Typedef* const * arguments = signature->getArgumentsType();
...
bool JavaJIT::analyzeForInlining(Reader& reader, uint32 codeLength) {
	...
	for (uint32 i = 0; i < codeLength; ++i) {
		...
		case LDC:
			i++;
			stack.push_back(bytecode);
			if (!canInlineLoadConstant(reader.readU1()))
				return false;
			break;
		...
		case LDC_W:
			i += 2;
			stack.push_back(bytecode);
			if (!canInlineLoadConstant(reader.readS2()))
				return false;
			break;
		...
		case DUP_X1: {
			uint8 one = stack.back();
			stack.pop_back();
			uint8 two = stack.back();
			stack.pop_back();
		...
		case PUTSTATIC: {
			...
			if (field == NULL)
				return false;
			if (needsInitialisationCheck(field->classDef))
				return false;
		...
		case INVOKEVIRTUAL: {
			...
			if (meth == NULL)
				return false;
			if (getReceiver(stack, meth->getSignature()) != ALOAD_0)
				return false;
			bool customized = false;
			if (!(isFinal(cl->access) || isFinal(meth->access))) {
				if (customizeFor == NULL)
					return false;
				if (!customizeFor->isSubclassOf(cl))
					return false;
				meth = customizeFor->lookupMethodDontThrow(meth->name,
						meth->type, false, true, NULL);
				assert(meth);
				assert(!meth->classDef->isInterface());
				assert(!isAbstract(meth->access));
				customized = true;
			}
			if (!canBeInlined(meth, customized))
				return false;
		...
		case INVOKESTATIC: {
			...
			if (!canBeInlined(meth, false))
				return false;
			if (needsInitialisationCheck(cl->asClass()))
				return false;
			updateStack(stack, meth->getSignature(), bytecode);
			break;
		}
