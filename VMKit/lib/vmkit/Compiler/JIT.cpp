...
cl::opt<bool> StandardCompileOpts("std-compile-opts",
...
// PassNameParser.
//
static llvm::cl::list<const llvm::PassInfo*, bool, llvm::PassNameParser> PassList(
		llvm::cl::desc("Optimizations available:"));

void VmkitModule::initialise(int argc, char** argv, bool isthisj3) {
	linkVmkitGC();
	...
	const char** llvm_argv = reinterpret_cast<const char**>(allocator.Allocate(
			(count + 7) * sizeof(char**)));
	int arrayIndex = 0;
...
void BaseIntrinsics::init(llvm::Module* module) {
	...
	pointerSizeType =
			module->getPointerSize() == Module::Pointer32 ?
					Type::getInt32Ty(Context) : Type::getInt64Ty(Context);

	UTF8SizeType = Type::getInt32Ty(Context);

	// Constant declaration
	constantLongMinusOne = ConstantInt::get(Type::getInt64Ty(Context),
			(uint64_t) -1);
	...
	constantMinusOne = ConstantInt::get(Type::getInt32Ty(Context),
			(uint64_t) -1);
	...
	constantMaxIntFloat = ConstantFP::get(Type::getFloatTy(Context),
			MaxIntFloat);
	constantMinIntFloat = ConstantFP::get(Type::getFloatTy(Context),
			MinIntFloat);
	constantMinLongFloat = ConstantFP::get(Type::getFloatTy(Context),
			MinLongFloat);
	constantMinLongDouble = ConstantFP::get(Type::getDoubleTy(Context),
			MinLongDouble);
	constantMaxLongFloat = ConstantFP::get(Type::getFloatTy(Context),
			MaxLongFloat);
	constantMaxIntDouble = ConstantFP::get(Type::getDoubleTy(Context),
			MaxIntDouble);
	constantMinIntDouble = ConstantFP::get(Type::getDoubleTy(Context),
			MinIntDouble);
	constantMaxLongDouble = ConstantFP::get(Type::getDoubleTy(Context),
			MaxLongDouble);
	constantMaxLongDouble = ConstantFP::get(Type::getDoubleTy(Context),
			MaxLongDouble);
	constantFloatInfinity = ConstantFP::get(Type::getFloatTy(Context),
			MaxFloat);
	constantFloatMinusInfinity = ConstantFP::get(Type::getFloatTy(Context),
			MinFloat);
	constantDoubleInfinity = ConstantFP::get(Type::getDoubleTy(Context),
			MaxDouble);
	constantDoubleMinusInfinity = ConstantFP::get(Type::getDoubleTy(Context),
			MinDouble);
	...
	constantThreadIDMask = ConstantInt::get(pointerSizeType,
			vmkit::System::GetThreadIDMask());
	constantStackOverflowMask = ConstantInt::get(pointerSizeType,
			Thread::StackOverflowMask);
	...
	constantPtrLogSize = ConstantInt::get(Type::getInt32Ty(Context),
			kWordSizeLog2);
	arrayPtrType = PointerType::getUnqual(
			ArrayType::get(Type::getInt8Ty(Context), 0));
...
Frames* VmkitModule::addToVM(VirtualMachine* VM, GCFunctionInfo* FI, JIT* jit,
		BumpPtrAllocator& allocator, void* meta) {
	...
	while (iterator.hasNext()) {
		...
		int i = 0;
		for (llvm::GCFunctionInfo::live_iterator KI = FI->live_begin(I), KE =
				FI->live_end(I); KI != KE; ++KI) {
