	...
	while (true) {
		...
		for (std::map<std::string, Patch*>::iterator itr = m_patchMap.begin();
			...
			if (globalval > 0
				...
				Gnuplotmsg *pMsgAvg = new Gnuplotmsg();
				pMsgAvg->msgtype = 3;
				pMsgAvg->value = (int)itr->second->avgexetime;
				pMsgAvg->functionindex = itr->second->functionindex;
				m_plotq.push(pMsgAvg);
			}
		...
		if (l_timegap > 20) {
			...
			for (std::map<std::string, Patch*>::iterator itr =
				...
				if(itr->second->samplingcount >0)
				{
					...
					void *FPtr = itr->second->llvmGlobalVariableMachinePointer;
					int (*FP)() = (int (*)())(intptr_t)FPtr;
					int samplingfrq = FP() - itr->second->globalvalueoffset;
					double frequency=0.0;
					if(samplingfrq !=0)
					    frequency = itr->second->avgexetime
							/ samplingfrq;
					...
					std::cout << "Function name - " << itr->first
							...
							<< frequency << " sec/call" << std::endl;


					pMsg->msgtype = 2;
					pMsg->value = 10*frequency;
					pMsg->functionindex = itr->second->functionindex;
					...
					itr->second->globalvalueoffset =FP();
					itr->second->avgexetime=0.0;
					itr->second->samplingcount=0;
					itr->second->previousexetime= rdtsc();
		...
		Class* customizeFor)
{
	...
	patchMap[key.c_str()]->llvmFunction->dump();
	std::cout<<" method name -"<<*methodname<<std::endl;
	func->dump();
