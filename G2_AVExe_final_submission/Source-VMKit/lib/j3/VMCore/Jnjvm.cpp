...
void UserClass::initialiseClass(Jnjvm* vm) {
	...
	assert(
			(isResolved() || getOwnerClass() || isReady() || isErroneous())
	...
	if (getInitializationState() != ready) {
		...
		JavaMethod* meth = lookupMethodDontThrow(
				vm->bootstrapLoader->clinitName,
				vm->bootstrapLoader->clinitType, true, false, 0);
		...
		if (JavaObject::getClass(exc)->isSubclassOf(
...
void Jnjvm::arrayStoreException() {
	error(upcalls->ArrayStoreException, upcalls->InitArrayStoreException,
			(JavaString*) 0);
}
...
void Jnjvm::indexOutOfBounds(const JavaObject* obj, sint32 entry) {
	...
	llvm_gcroot(obj, 0);
	llvm_gcroot(str, 0);
	str = (JavaString*) upcalls->IntToString->invokeJavaObjectStatic(this,
...
JavaObject* Jnjvm::CreateNegativeArraySizeException() {
	return CreateError(upcalls->NegativeArraySizeException,
			upcalls->InitNegativeArraySizeException, (JavaString*) 0);
}
...
JavaObject* Jnjvm::CreateUnsatisfiedLinkError(JavaMethod* meth) {
	...
	return CreateError(upcalls->UnsatisfiedLinkError,
			upcalls->InitUnsatisfiedLinkError, str);
}
...
JavaObject* Jnjvm::CreateOutOfMemoryError() {
	...
	return CreateError(upcalls->OutOfMemoryError, upcalls->InitOutOfMemoryError,
			str);
}
...
JavaObject* Jnjvm::CreateArrayStoreException(JavaVirtualTable* VT) {
	...
	llvm_gcroot(str, 0);
	if (VT != NULL)
		str = JavaString::internalToJava(VT->cl->name, this);
...
UserCommonClass* cl) {
	...
	return CreateError(upcalls->ClassCastException,
			upcalls->InitClassCastException, (JavaString*) 0);
}
...
JavaObject* Jnjvm::CreateLinkageError(const char* msg) {
	...
	llvm_gcroot(str, 0);
	str = asciizToStr(msg);
	return CreateError(upcalls->LinkageError, upcalls->InitLinkageError, str);
}
...
void Jnjvm::illegalAccessException(const char* msg) {
	...
	llvm_gcroot(str, 0);
	str = asciizToStr(msg);
	error(upcalls->IllegalAccessException, upcalls->InitIllegalAccessException,
			str);
}
...
void Jnjvm::illegalMonitorStateException(const JavaObject* obj) {
	...
	error(upcalls->IllegalMonitorStateException,
			upcalls->InitIllegalMonitorStateException, (JavaString*) 0);
}

void Jnjvm::interruptedException(const JavaObject* obj) {
	llvm_gcroot(obj, 0);
	error(upcalls->InterruptedException, upcalls->InitInterruptedException,
			(JavaString*) 0);
}
...
void Jnjvm::outOfMemoryError() {
	...
	llvm_gcroot(str, 0);
	str = asciizToStr("Java heap space");
	error(upcalls->OutOfMemoryError, upcalls->InitOutOfMemoryError, str);
}
...
void Jnjvm::classCastException(JavaObject* obj, UserCommonClass* cl) {
	llvm_gcroot(obj, 0);
	error(upcalls->ClassCastException, upcalls->InitClassCastException,
			(JavaString*) 0);
}
...
void Jnjvm::noClassDefFoundError(JavaObject* obj) {
	...
	errorWithExcp(upcalls->NoClassDefFoundError,
			upcalls->ErrorWithExcpNoClassDefFoundError, obj);
}
...
JavaString* CreateNoSuchMsg(CommonClass* cl, const UTF8* name, Jnjvm* vm) {
	ArrayUInt16* msg = NULL;
	JavaString* str = NULL;
	llvm_gcroot(msg, 0);
	llvm_gcroot(str, 0);
	msg = (ArrayUInt16*) vm->upcalls->ArrayOfChar->doNew(
			19 + cl->name->size + name->size, vm);

	uint32 i = 0;

	ArrayUInt16::setElement(msg, 'u', i);
	i++;
	ArrayUInt16::setElement(msg, 'n', i);
	i++;
	ArrayUInt16::setElement(msg, 'a', i);
	i++;
	ArrayUInt16::setElement(msg, 'b', i);
	i++;
	ArrayUInt16::setElement(msg, 'l', i);
	i++;
	ArrayUInt16::setElement(msg, 'e', i);
	i++;
	...
	ArrayUInt16::setElement(msg, ' ', i);
	i++;
	ArrayUInt16::setElement(msg, 'f', i);
	i++;
...
void Jnjvm::noSuchFieldError(CommonClass* cl, const UTF8* name) {
	...
	error(upcalls->NoSuchFieldError, upcalls->InitNoSuchFieldError, str);
}
...
void Jnjvm::noSuchMethodError(CommonClass* cl, const UTF8* name) {
	...
	llvm_gcroot(str, 0);
	str = CreateNoSuchMsg(cl, name, this);
	error(upcalls->NoSuchMethodError, upcalls->InitNoSuchMethodError, str);
}
...
void Jnjvm::abstractMethodError(CommonClass* cl, const UTF8* name) {
	...
	llvm_gcroot(str, 0);
	str = CreateNoSuchMsg(cl, name, this);
	error(upcalls->AbstractMethodError, upcalls->InitAbstractMethodError, str);
}
...
JavaString* CreateUnableToLoad(JavaString* name, Jnjvm* vm) {
	...
	for (sint32 j = name->offset; j < name->offset + name->count; ++j) {
		if (ArrayUInt16::getElement(JavaString::getValue(name), j) == '/') {
			...
			ArrayUInt16::setElement(msg,
					ArrayUInt16::getElement(JavaString::getValue(name), j), i);
			i++;
		}
	}
...
void Jnjvm::noClassDefFoundError(const UTF8* name) {
	...
	error(upcalls->NoClassDefFoundError, upcalls->InitNoClassDefFoundError,
			str);
}
...
void Jnjvm::classNotFoundException(JavaString* name) {
	...
	llvm_gcroot(str, 0);
	str = CreateUnableToLoad(name, this);
	error(upcalls->ClassNotFoundException, upcalls->InitClassNotFoundException,
			str);
}
...
void Jnjvm::noClassDefFoundError(UserClass* cl, const UTF8* name) {
	...
	ArrayUInt16::setElement(msg, ' ', i);
	i++;
	ArrayUInt16::setElement(msg, 'c', i);
	i++;
	ArrayUInt16::setElement(msg, 'l', i);
	i++;
	ArrayUInt16::setElement(msg, 'a', i);
	i++;
	ArrayUInt16::setElement(msg, 's', i);
	i++;
	ArrayUInt16::setElement(msg, 's', i);
	i++;
	ArrayUInt16::setElement(msg, ' ', i);
	...
	ArrayUInt16::setElement(msg, 'a', i);
	i++;
	ArrayUInt16::setElement(msg, 'm', i);
	i++;
	ArrayUInt16::setElement(msg, 'e', i);
	i++;
	ArrayUInt16::setElement(msg, 'd', i);
...
JavaString* Jnjvm::constructString(const ArrayUInt16* array) {
	...
	if (upcalls->internString) {
		return (JavaString*) upcalls->internString->invokeJavaObjectStatic(this,
				upcalls->internString->classDef, &key);
	} else {
		return key;
	}
}
...
JavaObject* UserCommonClass::getClassDelegatee(Jnjvm* vm, JavaObject* pd) {
	...
	if (getDelegatee() == NULL) {
		...
		if (pd == NULL && isArray()) {
			base =
					(JavaObjectClass*) asArrayClass()->baseClass()->getClassDelegatee(
							vm, pd);
			JavaObjectClass::setProtectionDomain(delegatee,
					JavaObjectClass::getProtectionDomain(base));
		} else {
...
JavaObject* const * UserCommonClass::getClassDelegateePtr(Jnjvm* vm,
		JavaObject* pd) {
	llvm_gcroot(pd, 0);
...
void ClArgumentsInfo::extractClassFromJar(Jnjvm* vm, int argc, char** argv,
	...
	ZipArchive* archive = new (allocator, "TempZipArchive") ZipArchive(bytes,
			allocator);
...
JnjvmClassLoader* Jnjvm::loadAppClassLoader() {
	...
	if (appClassLoader == NULL) {
		...
		loader = upcalls->getSystemClassLoader->invokeJavaObjectStatic(this,
				cl);
...
#undef LOAD_CLASS
	...
	UserCommonClass* math = loader->loadName(loader->mathName, true, true,
			NULL);
...
void Jnjvm::executeClass(const char* className, ArrayObject* args) {
	...
	TRY{
		...
		if (isPublic(method->access)) {
			std::cout<<"the method name which we invoke static -  " <<*method->getMehtodName()<<std::endl;
...
Jnjvm::Jnjvm(vmkit::BumpPtrAllocator& Alloc, vmkit::CompiledFrames** frames,
...
size_t Jnjvm::getObjectSize(gc* object) {
	...
	if (VMClassLoader::isVMClassLoader(src)) {
		...
		if (cl->isArray()) {
			...
			uint32 logSize =
					base->isPrimitive() ?
							base->asPrimitiveClass()->logSize :
							(sizeof(JavaObject*) == 8 ? 3 : 2);
...
void Jnjvm::printMethod(vmkit::FrameInfo* FI, word_t ip, word_t addr) {
	...
	fprintf(stderr, "; %p (%p) in %s.%s (line %d, bytecode %d, code start %p)",
			...
			UTF8Buffer(meth->name).cString(), meth->lookupLineNumber(FI),
